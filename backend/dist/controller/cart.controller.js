...
export const getCart = async (req, res) => {
    try {
        ...
        const totalItems = cart.items.reduce((sum, item) => sum + Number(item.quantity), 0);
        const totalValue = cart.items.reduce((sum, item) => sum + (Number(item.variant.price) * Number(item.quantity)), 0);
        return res.json({
            status: true,
            cart: {
                ...cart,
