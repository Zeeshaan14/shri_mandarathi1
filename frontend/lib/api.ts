export const UsersApi = {
  list: async (token?: string) => {
    const response = await apiFetch<{ status: boolean; users: any[] }>("/api/users", { token })
...
export const AuthApi = {
  login: (email: string, password: string) =>
    apiFetch<{ status: boolean; message: string; token: string; user: any }>("/api/auth/login", {
      method: "POST",
      body: { email, password },
    }),
  register: (name: string, email: string, password: string) =>
    apiFetch<{ status: boolean; message: string; user: any }>("/api/auth/register", {
      method: "POST",
...
export const ProductsApi = {
  list: async () => {
    const response = await apiFetch<{ status: boolean; products: any[] }>("/api/products")
    return response.products || []
  },
  categories: async () => {
    const response = await apiFetch<{ status: boolean; categories: any[] }>("/api/categories")
    return response.categories || []
  },
  addCategory: (name: string, token?: string) => apiFetch("/api/categories", { method: "POST", body: { name }, token }),
  updateCategory: (id: string, name: string, token?: string) =>
    apiFetch(`/api/categories/${id}`, { method: "PATCH", body: { name }, token }),
  deleteCategory: (id: string, token?: string) => apiFetch(`/api/categories/${id}`, { method: "DELETE", token }),
  ...
  deleteProduct: (id: string, token?: string) => apiFetch(`/api/products/${id}`, { method: "DELETE", token }),
}
...
export const CartApi = {
  ...
  removeItem: (cartItemId: string, token?: string) =>
    apiFetch(`/api/cart/item/${cartItemId}`, { method: "DELETE", token }),
  clear: (userId: string, token?: string) => apiFetch(`/api/cart/clear/${userId}`, { method: "DELETE", token }),
}

export const OrdersApi = {
  list: async (token?: string) => {
    const response = await apiFetch<{ status: boolean; orders: any[] }>("/api/orders", { token })
...
export const AddressesApi = {
  list: async (token?: string) => {
    const response = await apiFetch<{ status: boolean; addresses: any[] }>("/api/addresses", { token })
